...
actor MeetingCopyAudioReader {
    ...
    init(url: URL) throws {
        ...
        guard descriptor >= 0 else { throw MeetingCopyWire.Failure.invalid }
    ...
    private static func identity(_ handle: FileHandle) throws -> Identity {
        ...
        guard fstat(handle.fileDescriptor, &value) == 0,
              ...
              value.st_size <= MeetingCopyWire.audioLimit else { throw MeetingCopyWire.Failure.invalid }
    ...
    func verify(expected: MeetingCopyWire.Asset, frozenIdentity: Data? = nil) throws {
        ...
        while remaining > 0 {
            ...
            guard !bytes.isEmpty else { throw MeetingCopyWire.Failure.invalid }
            hash.update(data: bytes)
            remaining -= UInt64(bytes.count)
        }
        guard Data(hash.finalize()).base64EncodedString() == expected.sha256,
              try Self.identity(handle) == identity else { throw MeetingCopyWire.Failure.staleSnapshot }
    }

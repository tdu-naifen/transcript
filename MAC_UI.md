...
- macOS 与 iOS Simulator 的共享 CAM++ 引擎已用真实语音验证：5 秒独立同人片段
  正确匹配，未登记声音不冒用其他身份，改名、删会议及数据库重开后全局模板仍有效。
  短音频可能保持 Unknown，不因测试放宽阈值；结果见 [声纹 QA](SYNC_ACCEPTANCE.md)。
- Mac 客户端的 Voiceprints 资料展示、改名和同步与本地识别是不同能力。
  当前 Mac ASR 任务仍未调用 CAM++ 生成或识别声纹；下载模型和引擎测试通过
  不能当作这一产品入口已接通。此次 QA 不暗中扩大处理任务或声纹授权范围。

...
import Foundation
import OSLog
// Transcript adaptation: same-run evidence accessor and cancellation-safe preparation.
// Clustering algorithms and all upstream quality defaults are unchanged.
...
public final class OfflineDiarizerManager {
    ...
    public func prepare(
        ...
        do {
            (segmentationResult, embeddingResult) = try await withTaskCancellationHandler {
                async let awaitedSegmentation = segmentationTask.value
                async let awaitedEmbeddings = embeddingTask.value
                return try await (awaitedSegmentation, awaitedEmbeddings)
            } onCancel: {
                segmentationTask.cancel()
                embeddingTask.cancel()
                chunkContinuation.finish(throwing: CancellationError())
            }
        } catch {
            segmentationTask.cancel()
            embeddingTask.cancel()
            chunkContinuation.finish(throwing: error)
            _ = await segmentationTask.result
            _ = await embeddingTask.result
            throw error
        }
    ...
    public func cluster(_ prepared: PreparedDiarization) throws -> DiarizationResult {
        try clusterRun(prepared, evidenceOnly: false).0
    }

    /// Returns original masks and exact embedding/assignment correspondence. Skips
    /// legacy segment reconstruction (including its unsupported zero-vote fallback).
    public func clusterWithEvidence(_ prepared: PreparedDiarization) throws -> OfflineDiarizationEvidence {
    ...
    private func clusterRun(
        ...
        guard !timedEmbeddings.isEmpty else {
            if evidenceOnly {
                return (DiarizationResult(segments: []), OfflineDiarizationEvidence(
                    prepared: prepared, assignments: [], configuration: config, clusteringSeconds: 0))
            }
        ...
        let evidence = OfflineDiarizationEvidence(
            prepared: prepared, assignments: assignments, configuration: config,
            clusteringSeconds: Date().timeIntervalSince(clusteringStart))
        if evidenceOnly {
            return (DiarizationResult(segments: []), evidence)
        }

...
public struct SpeakerAnalysisRepository: Sendable {
    ...
    public func enqueue(meetingID: String, retry: Bool = false) async throws {
        try await database.writer.write { db in
            ...
            if retry {
                try db.execute(
                    sql:
                        "UPDATE speakerAnalysisJob SET state = CASE WHEN state IN ('running', 'rerun') THEN 'rerun' ELSE 'pending' END, error = NULL, updatedAt = ? WHERE meetingId = ?",
    ...
    public func nextPending() async throws -> String? {
        try await database.reader.read { db in
            try String.fetchOne(
                db,
                sql:
                    "SELECT meetingId FROM speakerAnalysisJob WHERE state IN ('pending', 'running', 'rerun') ORDER BY updatedAt, meetingId LIMIT 1"
            )
        }
    }
    ...
    public func failure(meetingID: String) async throws -> String? {
        try await database.reader.read { db in
            try String.fetchOne(
                db, sql: "SELECT error FROM speakerAnalysisJob WHERE meetingId = ? AND state = 'failed'", arguments: [meetingID])
        }
    ...
    public func apply(
        ...
        try await database.writer.write { db in
            try Self.apply(
                db, meetingID: meetingID, expectedUtterances: expectedUtterances,
                slotsByUtterance: slotsByUtterance, unknownUtteranceIDs: unknownUtteranceIDs,
                voices: voices, timeline: timeline, modelIdentifier: modelIdentifier, deviceID: deviceID,
                policy: policy, preprocessing: preprocessing)
        }
    }

    /// Shares the caller's transaction so model publication cannot leave identities
    /// or templates committed after its transcript/input fence has failed.
    @discardableResult
    public static func apply(
        ...
        guard unknownUtteranceIDs.isSubset(of: Set(current.map(\.id))),
            unknownUtteranceIDs.isDisjoint(with: slotsByUtterance.keys)
        else {
            throw VoiceprintBindingError.staleExpectation
        }
        ...
        for voice in voices.sorted(by: { $0.slot < $1.slot }) {
            ...
            if chosen == nil, let vector {
                ...
                if case .matched(let id, _) = policy.evaluate(
                    candidates: Array(candidates.prefix(2)), cleanDuration: voice.cleanDuration, generation: 0)
                {
                    chosen = id
                }
            }
            ...
            {
                let template = SpeakerEmbedding(
                    speakerId: speaker.id, floats: vector, originDeviceId: deviceID, modelIdentifier: modelIdentifier,
        ...
        for var utterance in current where assignUtterances {
            if let id = utterance.speakerId,
                try Speaker.fetchOne(db, key: id)?.displayName != nil
            {
                continue
            }
        ...
        try db.execute(
            sql:
                "UPDATE speakerAnalysisJob SET state = CASE WHEN state = 'rerun' THEN 'pending' ELSE 'complete' END, error = NULL, updatedAt = ? WHERE meetingId = ?",
            arguments: [Date(), meetingID])
        return resolved
    }
}
